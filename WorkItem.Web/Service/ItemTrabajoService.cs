...
namespace WorkItem.Web.Service
{
    public class ItemTrabajoService: IItemTrabajoService
    {
        ...
        public async Task<ResponseDto?> AsignarAsync(int id)
        {
            return await _baseService.SendAsync(new RequestDto()
            {
                ApiType = SD.ApiType.POST,
                Url = SD.ItemTrabajoAPIBase + "https://localhost:7248/api/ItemTrabajo/AsignarItemTrabajo/" + id
            });
        }
        ...
        public async Task<ResponseDto?> CreateItemTrabajoAsync(ItemTrabajoDto itemTrabajoDto)
        {
            return await _baseService.SendAsync(new RequestDto()
            {
                ApiType = SD.ApiType.POST,
                Data = itemTrabajoDto,
                Url = SD.ItemTrabajoAPIBase + "https://localhost:7248/api/ItemTrabajo/CreateItemTrabajo"
            });
        }
